...
// Common device types for initial search form
const DEVICE_TYPES = [
  ...
  return (
    <>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-1 bg-white/50 backdrop-blur-sm border-2 border-white/60 rounded-xl text-sm shadow-lg hover:shadow-xl transition-all focus:outline-none focus:border-black/30 text-left"
                        >
                          {/* Icon Square */}
                          <div className="w-10 h-10 flex-shrink-0 bg-white/40 backdrop-blur-sm border border-white/60 rounded-lg flex items-center justify-center">
                            {(() => {
                              const Icon = deviceType ? getDeviceIcon(deviceType) : Zap;
                              return <Icon className="w-5 h-5" strokeWidth={1.5} />;
                            })()}
                          </div>

                          {/* Device Type Text */}
                          <span className={deviceType ? "text-black" : "text-black/40"}>
                            {deviceType ? getDeviceType(deviceType)?.label : "Select device type..."}
                          </span>
                        </button>
                      </div>
      ...
      <DevicesModal
        ...
        onSelect={(selectedDeviceType) => setDeviceType(selectedDeviceType.value)}
