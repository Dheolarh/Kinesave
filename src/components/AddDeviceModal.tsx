    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          />
          <motion.div
            className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-2xl rounded-t-3xl z-50 px-6 py-6 max-h-[85vh] overflow-y-auto scrollbar-hide border-t border-white/60 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            ...
            {stage === "results" && (
              ...
              >
                <p className="text-sm text-black/60 mb-4">
                  Found {searchResults.length} device(s). Select one to add.
                </p>
                ...
                {error && (
                  <div className="text-sm text-red-500 bg-red-50 px-4 py-2 rounded-xl mb-4">
                    {error}
                  </div>
                )}
